...
num_classes = 106
num_train_samples = 19122
num_valid_samples = 3630
verbose = 1
batch_size = 16
num_epochs = 200
patience = 50
...
if __name__ == '__main__':
    ...
    # callbacks
    tensor_board = keras.callbacks.TensorBoard(log_dir='./logs_brand_back', histogram_freq=0, write_graph=True, write_images=True)
    log_file_path = 'logs_brand_back/training.log'
    csv_logger = CSVLogger(log_file_path, append=False)
    early_stop = EarlyStopping('val_acc', patience=patience)
    reduce_lr = ReduceLROnPlateau('val_acc', factor=0.1, patience=int(patience / 4), verbose=1)
    trained_models_path = 'models_brand_back/model'
